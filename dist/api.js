...
function routerFactory(client, authDataPath = '.wwebjs_auth') {
    // Create a router instance
    const router = express_1.default.Router();
    const whatsappService = new whatsapp_service_1.WhatsAppService(client);
    // Get the media storage path from the client configuration
    const mediaStoragePath = client.options?.mediaStoragePath ||
        '.wwebjs_auth/media';
    /**
     * @swagger
     * /api/status:
    ...
    router.get('/qr', (_req, res) => {
        ...
        if (!fs_1.default.existsSync(qrImagePath)) {
     ...
     *         description: Server error
     */
    router.post('/groups/:groupId/participants/add', async (req, res) => {
        try {
            const groupId = req.params.groupId;
     ...
     *         description: Server error
     */
    router.post('/messages/:messageId/media/download', async (req, res) => {
        try {
